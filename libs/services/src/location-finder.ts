import { Locations, LocationResult } from '@map-app/models';
...
export class LocationFinder {
  ...
  async call(): Promise<LocationResult[]> {
    const placesApi = process.env.PLACES_API || 'https://maps.googleapis.com/maps/api/place/textsearch/json?';
  ...
  static async call(location: string, currentLocation: string): Promise<LocationResult[]> {
